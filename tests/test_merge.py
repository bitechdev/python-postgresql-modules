...
class TestMergeOperations(unittest.TestCase):
    def test_proc_call_peek_file(self):
        filename = os.path.join(os.path.dirname(__file__), "data/Mail_Merge_Test_Template.docx")
        resultfile = os.path.join(os.path.dirname(__file__), "data/Mail_Merge_Test_Template_peek_result.json")
        print("Testing File: ", filename)
        
        [p_retval, p_errmsg, p_result, p_file] = execute_merge_sql("peek_docx", filename, None, None, None)
    ...
    def test_proc_call_peek_blob(self):
        ...
        with open(filename, "rb") as blobfile:
            ...
            [p_retval, p_errmsg, p_result, p_file] = execute_merge_sql("peek_docx", None, None, None, blobdata)
            print("Results", p_retval, p_errmsg)
